...
- Recommended pattern (from repository root):
  ...
  - `bin/dc exec --workspace-folder . bin/rails db:prepare`
...
- When outside Dev Container, run these via `bin/dc exec --workspace-folder . ...`.
...
- If running from host, execute tests via `bin/dc exec --workspace-folder .`.
